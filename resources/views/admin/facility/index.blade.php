              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <td>{{$facility->name}}</td>
                    <td>
                      <div style="width:36px;height:22px;border:1px solid #ddd;background: {{ $facility->color }};display:inline-block;vertical-align:middle;margin-right:6px;"></div>
                      <span class="text-muted">{{ $facility->color }}</span>
                    </td>
                    ...
                    <td>
                      @if($facility->status !== 'Closed')
                        <a href="{{route('facility.show',$facility->id)}}"   class="btn btn-sm btn-warning"><i class="fa fa-eye"></i></a>
                      @endif
                    @if(Auth::User()->role == 'BA' || Auth::user()->selectedRole->name == "Facility")
                          @if($facility->status !== 'Closed')
                              <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#addModal" data-id="{{$facility->id}}" data-name="{{$facility->name}}"
                                  data-max_booking="{{$facility->max_booking}}" data-per_user_max_booking="{{$facility->per_user_max_booking}}" data-gst="{{$facility->gst}}" data-status="{{$facility->status}}"
                                  data-icon="{{$facility->icon}}" data-color="{{$facility->color}}" data-booking_type="{{$facility->booking_type}}" data-closing_reason="{{$facility->closing_reason}}"><i class="fa fa-edit"></i></button>
                          @endif

                              {{-- Only show delete when facility is explicitly Closed and not soft-deleted --}}
                              @if(is_null($facility->deleted_at) && $facility->status === 'Closed')
