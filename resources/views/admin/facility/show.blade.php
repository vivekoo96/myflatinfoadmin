              ...
              <div class="card-body box-profile">
                ...
                <ul class="list-group list-group-unbordered mb-3">
                 ...
                 {{-- <li class="list-group-item">
                  ...
                  <li class="list-group-item">
                    <b>Status</b> <a class="float-right">{{$facility->status}}</a>
                  </li>
                </ul>
              </div>
               <!--/.card-body -->
              ...
              <div class="card-body">
                <div class="tab-content">
                  <div class="active tab-pane" id="timings">
                   @if(Auth::User()->role == 'BA' || Auth::user()->selectedRole->name == "Facility")
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td class="dates-column">
                            ...
                            @endphp
                            <div class="dates-display" title="{{ $timing->dates }}">
                              <span class="dates-short">
                                <strong>{{ $displayDates }}</strong>
                                @if($hasMoreDates)
                                  <span class="more-dates-btn" data-timing-id="{{ $timing->id }}" data-all-dates="{{ $allDatesFormatted }}">
                              ...
                              <span class="dates-full" style="display: none;">
                                <strong>{{ $allDatesFormatted ?? $displayDates }}</strong>
                                @if($hasMoreDates)
                                  <span class="less-dates-btn" data-timing-id="{{ $timing->id }}" style="color: #007bff; cursor: pointer; text-decoration: underline; margin-left: 5px;">
                          ...
                          <td class="timing-column"><strong>{{ \Carbon\Carbon::parse($timing->from)->format('h:i A') }}</strong></td>
                          <td class="timing-column"><strong>{{ \Carbon\Carbon::parse($timing->to)->format('h:i A') }}</strong></td>
                          <td>{{$timing->booking_option}}</td>
                          <td>
                            <span class="badge badge-{{ $timing->booking_type == 'Free' ? 'info' : 'success' }}">
                              {{$timing->booking_type}}
                            </span>
                          </td>
                          <td>
                            @if($timing->booking_type == 'Free')
                          ...
                          <td>
                            ...
                            @else
                              @if($timing->cancellation_type == 'Percentage')
                                {{$timing->cancellation_value}}%
                              @elseif($timing->cancellation_type == 'Fixed')
                                ₹{{$timing->cancellation_value}}
                              @else
                                {{$timing->cancellation_value}}
                              @endif
                        ...
                        <div class="btn-group" role="group">
                          <button class="btn btn-sm btn-primary edit-timing"
                            ...
                            data-id="{{$timing->id}}"
                            data-from="{{ \Carbon\Carbon::parse($timing->from)->format('H:i') }}"
                            data-to="{{ \Carbon\Carbon::parse($timing->to)->format('H:i') }}"
                            data-dates="{{$timing->dates}}"
                            data-status="{{$timing->status}}"
                            data-booking_option="{{$timing->booking_option}}"
                            data-booking_type="{{$timing->booking_type}}"
                            data-price="{{$timing->price}}"
                            data-cancellation_type="{{$timing->cancellation_type}}"
                            data-cancellation_value="{{$timing->cancellation_value}}"
                            title="Edit Timing">
                            <i class="fas fa-edit"></i>
                          </button>
                          @if($timing->deleted_at)
                              <button class="btn btn-sm btn-success" data-toggle="modal" data-target="#deleteModal" data-id="{{$timing->id}}" data-action="restore" title="Restore Timing"><i class="fas fa-undo"></i></button>
                          @else
                            <button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#deleteModal" data-id="{{$timing->id}}" data-action="delete" title="Delete Timing"><i class="fas fa-trash"></i></button>
                          @endif
                          <button class="btn btn-sm btn-warning open-cancel-modal" data-toggle="modal" data-target="#cancelModal" data-id="{{$timing->id}}" data-dates="{{ $timing->dates }}" title="Cancel Slot Bookings"><i class="fas fa-ban"></i></button>
                        </div>
                        @else
                  ...
                  <tr>
                    ...
                    <td class="booking-amount-column">
                      @if($booking->timing && $booking->timing->booking_type == 'Paid')
                        <span class="badge badge-success">₹{{ $booking->timing->price ?? 0 }}</span>
                        <br><small class="text-muted">{{ $booking->timing->booking_option ?? 'N/A' }}</small>
                      @elseif($booking->timing && $booking->timing->booking_type == 'Free')
                    ...
                    <td class="refunded-amount-column">
                      @if($booking->status == 'Cancelled' && $booking->timing && $booking->timing->booking_type == 'Paid')
                        @php
                        ...
                        @endphp
                        <span class="badge badge-warning">₹{{ number_format($refundAmount, 2) }}</span>
                        <br><small class="text-muted">{{ $booking->timing->cancellation_type }} Policy</small>
                      @elseif($booking->status == 'Cancelled' && $booking->timing && $booking->timing->booking_type == 'Free')
                    ...
                    <td class="cancelled-time-column">
                      @if($booking->status == 'Cancelled')
                        @if($booking->updated_at && $booking->updated_at != $booking->created_at)
                          <span class="badge badge-danger">{{ $booking->updated_at->format('M d, Y') }}</span>
                          <br><small class="text-muted">{{ $booking->updated_at->format('h:i A') }}</small>
                        @else
                          <span class="badge badge-secondary">N/A</span>
                    ...
                    <td>
                      @if($booking->status == 'Pending' && (Auth::User()->role == 'BA' || Auth::User()->hasRole('facility')))
                        <span class="badge badge-warning">{{ $booking->status }}</span>
                        <br><small class="text-muted">Awaiting Approval</small>
                      @else
                        <span class="badge badge-{{ $booking->status == 'Active' ? 'success' : ($booking->status == 'Cancelled' ? 'danger' : ($booking->status == 'Paused' ? 'warning' : 'secondary')) }}">
                          {{ $booking->status }}
                        </span>
                      @endif
                    </td>
                    <td class="cancellation-column" style="background-color: #f8f9fa; border: 1px solid #dee2e6;">
                      @if(isset($booking->timing) && $booking->timing)
                        <div class="cancellation-info">
                          <div class="mb-1">
                            <small class="text-muted">Type:</small> 
                            <span class="badge badge-info">{{ $booking->timing->cancellation_type ?? 'N/A' }}</span>
                          </div>
                          <div>
                            <small class="text-muted">Value:</small> 
                            <span class="badge badge-warning">
                              @if($booking->timing->cancellation_type == 'Percentage')
                                {{ $booking->timing->cancellation_value ?? 0 }}%
                              @elseif($booking->timing->cancellation_type == 'Fixed')
                                ₹{{ $booking->timing->cancellation_value ?? 0 }}
                              @elseif($booking->timing->cancellation_type == 'Manual')
                                Manual
                              @else
                                {{ $booking->timing->cancellation_value ?? 'N/A' }}
                              @endif
                            </span>
                          </div>
                        </div>
...
<div class="modal fade" id="cancelModal" tabindex="-1" role="dialog" aria-labelledby="cancelModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form action="{{ url('cancel-slot-booking') }}" method="post" class="cancel-form">
        @csrf
        <div class="modal-body">
          ...
          <input type="hidden" name="facility_id" id="cancel-facility_id" value="{{ $facility->id }}">
        </div>
