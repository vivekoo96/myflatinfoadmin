    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
                  ...
                  <div class="col-md-2">
                    <div class="form-group">
                      ...
                      <select name="shift" class="form-control">
                        ...
                        @foreach($shifts as $s)
                          <option value="{{ $s->name }}" {{ ($filters['shift'] ?? '') == $s->name ? 'selected' : '' }}>
                            {{ $s->name }}
                          </option>
                        @endforeach
                      </select>
                  ...
                  <div class="col-md-2">
                    <div class="form-group">
                      ...
                      <select name="checkin_type" class="form-control">
                        ...
                        <option value="photo" {{ ($filters['checkin_type'] ?? '') == 'photo' ? 'selected' : '' }}>Photo
                        </option>
          ...
          <div class="card">
            <div class="card-header with-border">
              <h3 class="card-title"><i class="nav-icon fas fa-list"></i> Patrol Records (Total: {{ $patrols->total() }})
              </h3>
            </div>
            <div class="card-body">
                  ...
                  <tbody>
                    ...
                    @forelse($patrols as $patrol)
                      ...
                      <tr>
                        <td>{{ $i }}</td>
                        <td>
                          {{ $patrol->guardUser ? ($patrol->guardUser->name ?? ($patrol->guardUser->first_name ?? '') . ' ' . ($patrol->guardUser->last_name ?? '')) : 'N/A' }}
                        </td>
                        ...
                        <td>
                          @if($patrol->photo_url)
                            <a href="{{ asset('public/images/' . $patrol->photo_url) }}" target="_blank">
                              <img src="{{ asset('public/images/' . $patrol->photo_url) }}"
                    ...
                    @empty
                      <tr>
                        <td colspan="7" class="text-center">No patrol records found</td>
                      </tr>
                    @endforelse
                  </tbody>
                </table>
