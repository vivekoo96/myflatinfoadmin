        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            ...
            @if(Auth::User()->role == 'BA')
                <button class="btn btn-sm btn-success right ml-2" data-toggle="modal" data-target="#addModal" {{ $active_count >= $login_limit ? 'disabled' : '' }}>Add New User</button>
            @endif
          </div>
        </div>
        <div class="row">
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">User List <span id="user-count-display" class="ml-2">{{ $total_count }} ({{ $active_count }} active)/{{$login_limit}}</span></h3>
              </div>
              <div class="card-body">
                    ...
                    <tbody>
                      ...
                      @forelse($users as $user)
                        ...
                        <td>{{$user->gender}}</td>
                          ...
                          <td>
                            @if($flatsCount > 0)
                              <span class="badge badge-primary">{{ $flatsCount }}</span>
                            @else
                          ...
                          <td>
                            ...
                            @if(Auth::User()->role == 'BA')
                              ...
                              @else
                                @if($flatsCount > 0)
                                  <button class="btn btn-sm btn-secondary" disabled title="User has {{ $flatsCount }} flat(s) — cannot delete"><i class="fa fa-trash"></i></button>
                                @else
    ...
    <div class="modal-content">
      ...
      <form action="{{url('store-user')}}" method="post" class="add-form">
        ...
        <div class="modal-body">
          ...
          {{--  <div class="form-group">
            <label for="building_select" class="col-form-label">Building:</label>
            <select name="building_id" id="building_select" class="form-control">
              <option value="">-- Select Building --</option>
              @php $buildings = \App\Models\Building::orderBy('name')->get(); $currentBuildingId = session('current_building_id') ?? (Auth::user() ? Auth::user()->building_id : null); @endphp
              @foreach($buildings as $bld)
                <option value="{{ $bld->id }}" {{ $currentBuildingId && $currentBuildingId == $bld->id ? 'selected' : '' }}>{{ $bld->name }}</option>
              @endforeach
            </select>
          </div> --}}

          {{-- <div class="form-group">
            <label for="flat_select" class="col-form-label">Flat:</label>
            <select name="flat_id" id="flat_select" class="form-control">
              <option value="">-- Select Flat --</option>
              @php $allFlats = \App\Models\Flat::with('building')->get(); @endphp
              @foreach($allFlats as $flat)
                <option value="{{ $flat->id }}" data-building-id="{{ $flat->building_id }}">{{ $flat->flat_no ?? $flat->name ?? 'Flat '.$flat->id }}</option>
              @endforeach
            </select>
          </div>
           --}}
...
<div class="modal fade" id="bulkUploadModal" tabindex="-1" role="dialog" aria-labelledby="bulkUploadModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{ url('bulk-upload-users') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="modal-body">
          <p>
            Please upload a CSV file with user data.  
            <a href="{{ url('download-sample-users') }}" class="btn btn-sm btn-link">Download Sample File</a>
          </p>
          <div class="form-group">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
          <button type="submit" class="btn btn-primary" {{ $active_count >= $login_limit ? 'disabled' : '' }}>Upload</button>
        </div>
      </form>
    </div>
  </div>
