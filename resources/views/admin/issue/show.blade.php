...
@section('content')
            ...
            <div class="card card-primary card-outline">
              <div class="card-body box-profile">
                <div class="text-center my-3">
                  @if(!empty($issue->photos) && isset($issue->photos[0]))
                ...
                <ul class="list-group list-group-unbordered mb-3">
                  ...
                  <li class="list-group-item">
                    <b>Status</b><a class="float-right">{{ ($issue->status === 'Solved' || $issue->status === 'Completed') ? 'Completed' : $issue->status }}</a>
                  </li>
                  @php
                  ...
                  <li class="list-group-item">
                    ...
                    @if($raisedByUser)
                    <a class="float-right">
                      <small>{{$raisedByUser->first_name}} {{$raisedByUser->last_name}}</small>
                      <br>
                      <small class="text-muted">({{$raisedByRole}})</small>
                    </a>
                    @else
                  ...
                  <li class="list-group-item">
                    ...
                    @if($assignedToUser)
                    <a class="float-right">
                      <small>{{$assignedToUser->first_name}} {{$assignedToUser->last_name}}</small>
                      <br>
    ...
    {{ ($issue->status === 'Solved' || $issue->status === 'Completed') ? 'Completed' : $issue->status }}
</span>
                          ...
                          <div class="col-md-4 mb-3">
                            <div class="card shadow-sm h-100" style="cursor: pointer; overflow: hidden; border-radius: 8px; transition: transform 0.3s, box-shadow 0.3s;">
                              <div class="image-container" style="height: 200px; overflow: hidden; background: #f5f5f5; display: flex; align-items: center; justify-content: center;">
                                <img src="{{$photo->photo}}" class="img-fluid" style="height: 100%; width: 100%; object-fit: cover;" data-toggle="modal" data-target="#imageModal" data-image="{{$photo->photo}}" alt="Issue photo">
                              </div>
                      ...
                      <div class="mt-4">
                          ...
                          <div style="max-height: 400px; min-height: 300px; overflow-y: scroll; border: 2px solid #007bff; border-radius: 8px; background: #f4f8ff; box-shadow: 0 2px 8px rgba(0,0,0,0.04); display: flex; align-items: center; justify-content: center;">
                            <ul id="commentList" class="list-unstyled mt-3 w-100">
                             @forelse($issue->comments->sortBy('created_at') as $comment)
                                  ...
                                  <li class="media mb-3" data-id="{{$comment->id}}">
                                      ...
                                      @endphp
                                      <img class="mr-3 rounded-circle" src="{{$commentPhoto}}" width="50" height="50">
                                      <div class="media-body">
                                         @php
                                            ...
                                            @endphp
                                            <small class="mt-0 mb-1"><strong>{{$displayUser->first_name ?? ''}} {{$displayUser->last_name ?? ''}} - {{$comment->commentedbyRole}}</strong></small>
                                          ...
                                          <ul class="list-unstyled ml-4 mt-2">
                                              @forelse($comment->replies as $reply)
                                                  <li class="media mb-2">
                                                      ...
                                                      @endphp
                                                      <img class="mr-3 rounded-circle" src="{{$replyPhoto}}" width="30">
                                                      <div class="media-body">
                                                            @php
                                                            ...
                                                            @endphp
                                                            <h6 class="mt-0 mb-1">{{$reply->user->first_name ?? ''}} {{$reply->user->last_name ?? ''}} ({{$reply->user->email ?? ''}}) - {{$replyRoleName}}</h6>
                                                          <p>{{$reply->text}}</p>
...
$(document).ready(function() {
  ...
  $("form[action='{{ url('add-comment') }}']").on("submit", function(e) {
