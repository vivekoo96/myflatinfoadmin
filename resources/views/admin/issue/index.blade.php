            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    <td>{{$i}}</td>
                    <td>{{$issue->flat ? $issue->flat->name : 'N/A'}}</td>
                    <td>{{$issue->block ? $issue->block->name : 'N/A'}}</td>
                    <td>{{$issue->created_by_rolename}}</td>
                    <td>{{$issue->department->name}}</td>
                    <td>
                        @if(!empty($issue->photos) && isset($issue->photos[0]))
                          
                            <a href="{{ $issue->photos[0]->photo }}"  style="text-decoration: underline; color: #007bff; font-size: 12px;">View Image</a>
                          @else
                          <span>No Image</span>
                        @endif
                    </td>
                    ...
                    <td>{{ ($issue->status === 'Solved' || $issue->status === 'Completed') ? 'Completed' : $issue->status }}</td>
                    <td>
                      <a href="{{route('issue.show',$issue->id)}}"   class="btn btn-sm btn-warning"><i class="fa fa-eye"></i></a>
                      @if(Auth::User()->role == 'BA' || Auth::user()->selectedRole->name == "Issue Tracker")
