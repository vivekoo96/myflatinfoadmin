          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                ...
                <button class="btn btn-sm btn-success right ml-2" data-toggle="modal" data-target="#addModal" {{ $created_counts >= $flat_limit ? 'disabled' : '' }}>Add New Flat</button>
                  ...
                  <tr>
                    ...
                    <td>{{$flat->name}}</td>
                     ...
                     @php
                        ...
                        @endphp
<td>
@if($flat->owner && $ownerBuildingUserId)
    <a href="{{ url('user/'.$flat->owner->id.'/'.$ownerBuildingUserId) }}" target="_blank">
        {{ $flat->owner->name }}
    </a>
@else
    —
...
<td>
@if($flat->tanent && $tenantBuildingUserId)
    <a href="{{ url('user/'.$flat->tanent->id.'/'.$tenantBuildingUserId) }}" target="_blank">
        {{ $flat->tanent->name }}
    </a>
@else
    —
@endif
                    ...
                    <td class="d-flex">
            <a href="{{route('flat.show',$flat->id)}}"   class="btn btn-sm btn-warning mx-1"><i class="fa fa-eye"></i></a>
            @if(Auth::User()->role == 'BA')
            <button class="btn btn-sm btn-primary mx-1" data-toggle="modal" data-target="#addModal" data-id="{{$flat->id}}" data-name="{{$flat->name}}" data-status="{{$flat->status}}" data-sold_out="{{$flat->sold_out}}" 
         ...
         @if(Auth::User()->building->hasPermission('Corpusfund'))
                @if(Auth::User()->role == 'BA' || Auth::User()->hasRole('accounts'))
                    <button class="btn btn-sm btn-primary mx-1" data-toggle="modal" data-target="#corpusModal" data-id="{{$flat->id}}" data-corpus-fund="{{$flat->corpus_fund}}" data-is_corpus_paid="{{$flat->is_corpus_paid}}" 
            ...
            @if(Auth::User()->role == 'BA')
              @if($flat->deleted_at)
                <button class="btn btn-sm btn-success mx-1" data-toggle="modal" data-target="#deleteModal" data-id="{{$flat->id}}" data-action="restore"><i class="fa fa-undo"></i></button>
              @else
                <button class="btn btn-sm btn-danger mx-1" data-toggle="modal" data-target="#deleteModal" data-id="{{$flat->id}}" data-action="delete"><i class="fa fa-trash"></i></button>
              @endif
            @endif
                    </td>
...
<div class="modal fade" id="bulkUploadModal" tabindex="-1" role="dialog" aria-labelledby="bulkUploadModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form action="{{ url('bulk-upload-flats') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="modal-body">
          <div class="row">
            <div class="col-md-8">
            ...
            <div class="col-md-4">
              <div class="text-center">
                <a href="{{ url('download-sample-flats') }}" class="btn btn-success btn-block">
                    ...
                    <ul class="mb-0">
                      <li><strong>Block Names:</strong> 
                        @if(isset($blocks) && count($blocks) > 0)
                          {{ implode(', ', $blocks->pluck('name')->toArray()) }}
                        @else
            ...
            <div class="col-md-6">
              <div class="card">
                <div class="card-body text-center">
                  ...
                  <p class="mb-1"><strong>Flats:</strong> {{ $created_counts }} / {{ $flat_limit }}</p>
                  <p class="mb-0"><strong>Available:</strong> {{ $flat_limit - $created_counts }} flats</p>
                </div>
              </div>
            </div>
            <div class="col-md-6">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary" {{ $created_counts >= $flat_limit ? 'disabled' : '' }}>
  ...
  $(document).ready(function(){
    ...
    $('#addModal').on('show.bs.modal', function (event) {
      ...
      if(edit_id) {
        ...
        $('#building_id').val(button.data('building_id') || '{{ $building->id }}');
